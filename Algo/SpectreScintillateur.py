def spectre():
    ...
    while delta_t<t_max:
            ...
            t = line.split()
            if flag==False:
                t_ref = int(t[0],16)#le temps de reference est celui de la premiere ligne lue
                #print(t_ref)
                flag=True #on ne prend qu une fois le temps de reference
            if flag==True:
                delta_t = int(t[0],16)- t_ref
            for k in range(1,17):
